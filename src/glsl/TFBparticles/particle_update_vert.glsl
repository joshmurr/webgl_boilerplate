...
void main(){
    if(i_Age >= i_Life) {
        ...
        // every time.
        vec2 noise_coord = vec2(gl_VertexID % 512, gl_VertexID / 512);
        ...
        float theta = u_MinTheta + random(noise_coord)*(u_MaxTheta - u_MinTheta);

        float x = cos(theta);
        float y = sin(theta);

        v_Position = u_Origin;
        // v_Position = vec2((random(noise_coord)*2.0)-1.0, (random(noise_coord)*2.0)-1.0);

        v_Age = 0.0;
        v_Life = i_Life;

        v_Velocity = vec2(x, y) * (u_MinSpeed + random(noise_coord) * (u_MaxSpeed - u_MinSpeed));
    } else {
